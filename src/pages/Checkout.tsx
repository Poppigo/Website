...
import { Loader2, ChevronRight, Tag, X, Banknote, CreditCard, Smartphone, Building2, Zap, Wallet } from "lucide-react";
...
const Checkout = () => {
  ...
  const [paymentMethod, setPaymentMethod] = useState<"upi" | "netbanking" | "card" | "emi" | "wallet" | "cod">("card");
  ...
  const codTotal = finalTotal + COD_FEE;
    ...
    return (
      <div className="min-h-screen bg-peach">
        ...
        <div className="pt-24 pb-20 flex flex-col items-center justify-center min-h-[60vh] text-center px-6">
          <h1 className="font-body text-3xl md:text-5xl font-bold text-foreground mb-3">No items to checkout</h1>
          <Link to="/shop" className="px-8 py-3 rounded-full font-semibold hover:opacity-90 transition-opacity" style={{ backgroundColor: '#ccff00', color: '#1b2a54' }}>
            Shop Now
          </Link>
        </div>
        <Footer />
    ...
    <div className="min-h-screen bg-peach">
      ...
      <div className="pt-24 pb-20">
        <div className="max-w-6xl mx-auto px-6">
          ...
          <h1 className="font-body text-4xl md:text-5xl font-bold text-foreground mb-10">Checkout</h1>
          ...
          <div className="grid grid-cols-1 lg:grid-cols-5 gap-10">
            ...
            <div className="lg:col-span-3">
              ...
              >
                <h2 className="font-body text-2xl md:text-3xl font-bold text-foreground mb-6">Billing Details</h2>
            ...
            <div className="lg:col-span-2">
              ...
              >
                <h2 className="font-body text-2xl md:text-3xl font-bold text-foreground mb-6">Your Order</h2>

                <div className="space-y-3 mb-6">
                  {items.map((item) => (
                    <div key={`${item.id}__${item.size || ""}`} className="flex items-center justify-between text-base">
                ...
                <div className="border-t border-border pt-4">
                  <h3 className="text-base font-body font-semibold mb-3" style={{ color: '#1b2a54' }}>Payment Method</h3>
                  <div className="flex flex-col gap-2">
                    {[
                      ...
                      const isCod = id === "cod";
                      const displayAmount = isCod ? codTotal : finalTotal;
                      return (
                        <div key={id} className="relative">
                          {!isCod && (
                            <span className="absolute -top-2.5 left-4 z-10 text-sm font-bold px-2.5 py-0.5 rounded-full" style={{ backgroundColor: '#22c55e', color: '#fff' }}>Save ₹{COD_FEE}</span>
                          )}
                          {isCod && (
                            <span className="absolute -top-2.5 left-4 z-10 text-sm font-bold px-2.5 py-0.5 rounded-full" style={{ backgroundColor: '#ef4444', color: '#fff' }}>+₹{COD_FEE} COD fee</span>
                          )}
                          <button
                            type="button"
                            onClick={() => setPaymentMethod(id as any)}
                            className="flex items-center justify-between px-4 py-3.5 rounded-xl border-2 text-base font-semibold transition-all w-full"
                            style={paymentMethod === id
                              ? { borderColor: '#ccff3c', backgroundColor: '#f4ffe6', color: '#1b2a54' }
                              : { borderColor: '#e5e7eb', backgroundColor: '#ffffff', color: '#374151' }}
                          >
                            <div className="flex items-center gap-3">
                              <Icon size={18} style={{ color: paymentMethod === id ? '#4241ff' : '#9ca3af' }} />
                              <span>{label}</span>
                            </div>
                            <div className="flex items-center gap-2">
                              <span className="font-bold" style={{ color: paymentMethod === id ? '#FF6B35' : '#1b2a54' }}>
                ...
                <div className="border-t border-border pt-4 space-y-2">
                  <div className="flex justify-between text-base">
                    ...
                    <span className="font-medium" style={{ color: '#1b2a54' }}>₹{totalPrice.toFixed(2)}</span>
                  </div>
                  {discount > 0 && (
                    <div className="flex justify-between text-base">
                      <span style={{ color: '#22c55e' }}>Discount</span>
                      <span className="font-medium" style={{ color: '#22c55e' }}>−₹{discount.toFixed(2)}</span>
                    </div>
                  )}
                  {paymentMethod === 'cod' && (
                    <div className="flex justify-between text-base">
                      <span className="text-muted-foreground">COD Fee</span>
                      <span className="font-medium" style={{ color: '#1b2a54' }}>+₹{COD_FEE.toFixed(2)}</span>
                    </div>
                  )}
                  <div className="flex justify-between text-lg font-bold border-t border-border pt-3 mt-3">
                    <span style={{ color: '#1b2a54' }}>Total</span>
                    <span style={{ color: '#FF6B35' }}>₹{(paymentMethod === 'cod' ? codTotal : finalTotal).toFixed(2)}</span>
                  </div>
                </div>
                ...
                <button
                  ...
                  disabled={processing}
                  className="w-full mt-6 px-6 py-4 rounded-full font-bold text-lg hover:opacity-90 transition-opacity flex items-center justify-center gap-2 disabled:opacity-50"
                ...
                >
                  {processing ? (
                    ...
                    <>
                      {paymentMethod === "cod" && <Banknote size={18} />}
