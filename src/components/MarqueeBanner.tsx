...
const MarqueeBanner = () => {
  ...
  return (
    <section className="py-6 md:py-8 overflow-hidden" style={{ backgroundColor: "#0f1a35" }}>
        ...
        {repeated.map((item, i) => (
          <span key={i} className="flex items-center gap-6">
            <span className="min-w-[190px] md:min-w-[220px] h-[110px] md:h-[128px] px-5 md:px-6 rounded-[1.75rem] bg-white/8 border border-white/10 flex flex-col justify-center whitespace-normal">
              {item.map((line) => (
                <span key={line} className="font-body font-bold text-lg md:text-2xl lg:text-[1.8rem] uppercase tracking-wide leading-[1.05]" style={{ color: '#ccff3c' }}>
                  {line}
                </span>
              ))}
            </span>
            <span className="text-2xl md:text-3xl" style={{ color: '#ccff3c' }}>✦</span>
          </span>
        ))}
      </div>
    </section>
